...
export default function ResumeUpload() {
    ...
    <section className="w-full flex justify-center items-center py-12 pt-20">
        <div className="w-full max-w-5xl mx-auto bg-gradient-to-br from-[#173465] via-[#244865] to-[#2F3E6D] rounded-[24px] p-10 shadow-xl">
            {/* Header */}
            <div className="flex items-baseline gap-2 mb-6">
            <Upload className="w-5 h-5 text-white" />
            <h2 className="text-white text-2xl font-bold font-sans">Upload your resume</h2>
            </div>
            ...
            className={`relative w-full min-h-[220px] border-2 border-dashed p-8 text-center transition-all duration-200 cursor-pointer
                bg-gradient-to-r from-[#1C4278] via-[#1F476A] to-[#343C74]
                rounded-2xl shadow-lg border-[#274879]
                ${dragActive ? 'ring-2 ring-blue-400 ' : 'hover:border-white/10 hover:translate-y-1'}`}
            >
            <input
                id="resume-upload"
                type="file"
            ...
            <div className="space-y-3 mt-6">
                ...
                {resumeFile ? (
                    <div>
                    <p className="text-white/80 mt-1">📄 {resumeFile} uploaded successfully</p>
                    <p className="text-blue-200 text-sm mt-2">Click to change file</p>
                    </div>
                ) : (
                    <div>
                    <h3 className="text-white font-semibold">Drag & Drop your Resume</h3>
                    <p className="text-blue-200 mt-2">Supports PDF, DOC, DOCX up to 10MB</p>
                    <p className="text-blue-300 text-sm mt-2 mb-8">Click to browse files from device</p>
                    </div>
                )}
                </label>
            </div>
            ...
            <h3 className="text-white font-semibold text-lg">Target Job Description</h3>
            </div>

            {/* …your textarea + button here… */}
        </div>
    </section>
